...
def index(request):
    message = "Stock Does Application"
    return render(request, 'stocks/index.html', {'message': message})
...
def stock_list(request):
    ...
    #else :
    return render(request, 'stocks/stock_list.html', {'info': 'No data'})
'''
...
def upload_csv(request):
    if request.method == 'POST':
        ...
        if forms.is_valid():
            ...
            try:
                # Read each row in the CSV file and add it to the list
                for row in reader:
                    data.append(row)
                    ...
                    stock, created = Stock.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            'market_cap': float(row[2]) #'MarketCap(ล้านบาท)'])
                        })
            ...
            return render(request,'stocks/upload_csv.html',{'info':data})  
        return render(request, 'stocks/upload_csv.html', {
                                'data': data,
                            })
    else:
